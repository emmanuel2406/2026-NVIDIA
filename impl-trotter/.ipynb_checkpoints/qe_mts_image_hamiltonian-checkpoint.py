...
def sample_quantum_population(N: int, n_shots: int, trotter_steps: int,
    ...
    if verbose:
        print(f"[QUANTUM] Preparing circuit: N={N}, shots={n_shots}, steps={trotter_steps}, T={total_time}")

    # Get Hamiltonian terms
    t1, t2, t3, t4 = get_image_hamiltonian(N)

    if verbose:
        print(f"[QUANTUM] Hamiltonian terms: {len(t1)} (1-body), {len(t2)} (2-body), "
              f"{len(t3)} (3-body), {len(t4)} (4-body)")

    # Convert to float lists for CUDA-Q
    ...
    # Compute annealing schedule (sinusoidal)
    ...
    print(cudaq.draw(dcqo_flexible_circuit_v2, N, trotter_steps,
    ...
    # Convert quantum samples to population
    population = []
    energies = []
...
def sample_quantum_population_labs(N: int, n_shots: int, trotter_steps: int,
    ...
    # Get LABS Hamiltonian terms (returns t1=[], t2, t3=[], t4)
...
def print_final_summary(results: List[BenchmarkResult], config: RunConfig):
    ...
    for r in results:
        ...
        print(f"  Sequence:      {r.best_sequence[:40]}...")
